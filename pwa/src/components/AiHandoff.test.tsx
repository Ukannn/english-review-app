...
it("freezes the actual confirmation decisions and permits an independent correction",async()=>{
 ...
 const api:ApiClient={...demoApi,getSubmissionStatus:vi.fn(async()=>status),confirmGrades:vi.fn(async()=>({ok:true}))};const user=userEvent.setup();render(<AiHandoff api={api} submissionId="submission" onDone={()=>undefined} onClose={()=>undefined}/>);await screen.findByText("第 1 题");await user.click(screen.getByText("调整此题批改"));await user.selectOptions(screen.getByLabelText("目标提取"),"not_measured");await user.click(screen.getByRole("button",{name:"确认以上批改并更新复习安排"}));await waitFor(()=>expect(api.confirmGrades).toHaveBeenCalled());const args=vi.mocked(api.confirmGrades).mock.calls[0];expect(args[1][0]).toMatchObject({decision:"accept",targetOutcome:"not_measured",meaningOk:true});expect(args[4]).toBe(await sha256Jsonb(args[1]));
});
